...
char* indexes(const char* path)
{
    char* p = malloc(strlen(path) * sizeof(path) + 11);
    printf("indexes: path= %s\n", path);
    strcpy(p, path);
    strcat(p, "index.php");
    printf("indexes: p= %s\n", p);
    FILE* f = fopen(p, "r");
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    size_t size = 0;
    BYTE dummy;
    while(fread(&dummy, sizeof(BYTE), 1, file) == 1)
        size++;
    fseek(file, 0, SEEK_SET);
    BYTE *hspace = malloc(size * sizeof(BYTE));
    if(hspace == NULL) {
        printf("Not allocated!\n");
        return false;
    }
    if(fread(hspace, size * sizeof(BYTE), 1, file) != 1) {
        return false;
    }
    *length = size;
    *content = hspace;
...
const char* lookup(const char* path)
{
    ...
    int len = strlen(path);
    char* pcopy = malloc(len * sizeof(char) + 1);
    pcopy = strcpy(pcopy, path);
    for(int i = 0; i < len; i++)
        pcopy[i] = tolower(pcopy[i]);
    if(strstr(path, ".css"))
        return "text/css";
...
{
    printf("684 parse: %s\n", line);
    char *s = strchr(line, '/'), *i = s;
    char *q = strchr(line, '?');
    if(strstr(line, "GET") == NULL || strstr(line, "GET") != line || *(line + 3) != ' ') {
        error(405);
        return false;
    }
    else if(line[4] != '/') {
    ...
    else if(strstr(line, "HTTP/1.1") != NULL && *(strstr(line, "HTTP/1.1") - 2) == ' ') {
        error(400);
        return false;
    }
    else {
